...
<head>
    ...
    <style>
        ...
        .dashboard-card { border-left: 4px solid transparent; }
        ...
        .dropdown-content { display: none; position: absolute; background-color: #f9f9f9; min-width: 160px; box-shadow: 0px 8px 16px 0px rgba(0,0,0,0.2); padding: 8px 0; z-index: 1; right: 0; }
...
<div class="flex h-screen">
    ...
    <div class="flex-1 overflow-y-auto p-4">
        ...
        <header class="mb-4">
            <div class="flex justify-between items-center">
                ...
                <div class="flex items-center space-x-4">
                    <!-- Ajout d'un container flex et espace entre les boutons -->
                    <div class="dropdown">
        ...
        <section id="budget-section" class="hidden">
            <button onclick="openModal('calculateBudgetModal')" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">
                Calculer le budget 50/30/20
            </button>
        </section>
...
<!-- Modal pour calculer le budget 50/30/20 -->
<div id="calculateBudgetModal" class="modal">
    <div class="modal-content">
        <span class="close" onclick="closeModal('calculateBudgetModal')">×</span>
        <h3>Calculer le budget 50/30/20</h3>
        <form id="calculateBudgetForm" action="{{ route('budget-optimisations.calculate') }}" method="POST">
            @csrf
            <div class="mb-4">
                <label for="income" class="block text-gray-700 text-sm font-bold mb-2">Revenu mensuel :</label>
                <input type="number" name="income" id="income" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required step="0.01">
            </div>
            <button type="submit" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">
                Calculer
            </button>
        ...
        <div id="budgetResult" class="mt-4">
            <!-- Le résultat du calcul sera affiché ici -->
        </div>
    </div>
</div>
...
<script>
    function openModal(modalId, action, id = null, name = null, goal_amount = null, deadline = null) {
        ...
        } else if (modalId === 'calculateBudgetModal') {
            // No specific initialization needed for this modal
        }
  ...
  function deleteTransaction(id) {
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      const calculateBudgetForm = document.getElementById('calculateBudgetForm');
        if (calculateBudgetForm) {
            calculateBudgetForm.addEventListener('submit', function(event) {
                event.preventDefault();
                ...
                .then(response => response.text()) // Récupérer le HTML
                .then(data => {
                    document.getElementById('budgetResult').innerHTML = data; // Afficher le HTML dans le modal
                })
                .catch(error => {
                    console.error('Error:', error);
                    document.getElementById('budgetResult').innerHTML = '<p>Une erreur s\'est produite lors du calcul du budget.</p>';
                });
            });
        }
    });
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Extraire les labels et les valeurs
                // pour les dépenses et les revenus
